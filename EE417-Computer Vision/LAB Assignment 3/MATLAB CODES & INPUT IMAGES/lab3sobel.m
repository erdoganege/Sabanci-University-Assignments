function new_img_Edges = lab3sobel(img, threshold)
...
new_img_X = uint8(new_img_data_X);
new_img_Y = uint8(new_img_data_Y);

new_img_data_G = sqrt(new_img_data_X.^2 + new_img_data_Y.^2);
new_img_G = uint8(new_img_data_G);

[row2,col2,ch2] = size(new_img_data_G);
new_img_data_Edges = zeros(row2,col2); 

indexes = find(new_img_data_G>threshold);

new_img_data_Edges = reshape(new_img_data_Edges, [row2*col2, 1]);
new_img_data_Edges(indexes) = 255;
...
title("Sobel Edges with threshold " + threshold);
